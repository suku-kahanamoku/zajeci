...
export default defineNuxtModule<ModuleOptions>({
  ...
  async setup(_options, _nuxt) {
    ...
    GENERATE_PAGES("/admin/text", resolve);

    // Public GET endpoints
    const apiTextDir = resolve("./runtime/server/api/text");
    fs.readdirSync(apiTextDir)?.forEach((file) => {
      GENERATE_API_ENDPOINT(file, "/api/text", resolve);
    });

    // Admin write endpoints (POST, PATCH, DELETE)
    const apiAdminTextDir = resolve("./runtime/server/api/admin/text");
    fs.readdirSync(apiAdminTextDir)?.forEach((file) => {
      GENERATE_API_ENDPOINT(file, "/api/admin/text", resolve);
    });
