...
import aConfig from "../../../assets/configs/admin-address-list.json";

definePageMeta({
  layout: "admin",
  syscode: "admin_address",
  title: "$.address.admin.title",
});
...
<template>
  <div v-if="config" :id="config.syscode" class="w-full max-w-7xl mx-auto px-5">
    ...
    <div class="flex justify-end gap-2">
      ...
      />
      <UButton
        :to="localePath(routes.admin_address_create?.path!)"
    ...
    >
      {{
        selected?.length > 1
          ...
          : $tt("$.message.delete_question", { name: selected?.[0]?.street })
      }}
    </CmpConfirmDialog>
  </div>
</template>
