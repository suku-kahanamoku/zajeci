...
import iConfig from "../../../assets/configs/admin-invoice-list.json";

definePageMeta({
  layout: "admin",
  syscode: "admin_invoice",
...
<template>
  <div v-if="config" :id="config.syscode" class="w-full max-w-7xl mx-auto px-5">
    ...
    <div class="flex justify-end gap-2">
      ...
      />
      <UButton
        :to="localePath(routes.admin_invoice_create?.path!)"
    ...
    >
      {{
        selected?.length > 1
          ...
          : $tt("$.message.delete_question", { name: selected?.[0]?.invoice_number })
      }}
    </CmpConfirmDialog>
  </div>
</template>
